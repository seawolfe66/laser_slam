...
class PathPlannerNode(object):
    ...
    def __init__(self):
        ...
        self.path_pub = rospy.Publisher('visualization_path',Path,queue_size=100)
        # Setup initial variables
        
        #Setup service for gmapping 
        rospy.Service('provide_line', bowstatesResponse, self.get_start_end_line)
        self.bow_pub = rospy.Publisher('bow_command',String,queue_size=10)
        #
        self.done_pub = rospy.Publisher('heatmap_area_done',String,queue_size=10)
        ...
        self.timeout = False
        self.scan_front_dist = 0
         # Create the actionlib service
        self.move_base_client = SimpleActionClient('move_base', MoveBaseAction)

        # subscribe scan topic for detecting obstacle ahead
        ...
        while not rospy.is_shutdown():
            ...
            if self.start_path_following:
                ...
                if not self.setup_path_following(heading)   :
                    continue  
    ...
    def field_callback(self, msg):
        ...
        rospy.loginfo("Handle the message of polygon with size ")

    def isObstacle(self):
        if self.scan_front_dist < 0.3:
            return True
        else :
            return False
    def scan_callback(self, msg):
        """
        Receive the message from laser scanning , which is used to judge the distance to the obstacle ahead 
        """
        size = len(msg.ranges)
        front_index = -1
        dis_front = 0
        for index, range in enumerate(msg.ranges):
            ...
            if angle >= -10*msg.angle_increment and angle <= 10*msg.angle_increment :
                if front_index == -1 or dis_front > range:
                    dis_front = range
                    front_index = index 
        
        self.scan_front_dist = msg.ranges[front_index]

        rospy.loginfo("Scan callback %.2f " % self.scan_front_dist)
        if self.isObstacle() :
            self.move_base_client.cancel_all_goals()
            self.done_pub.publish("Obstacle")
    ...
    def setup_path_following(self, degrees=0):
       ...
       # self.goal_rotate()
        ...
        if not self.plan_path(self.field_shape, origin, degrees) :
                rospy.logwarn("not a valid path ")
                return False
        ...
        while not connected_to_move_base:
            # Wait for the server for a while
            connected_to_move_base = self.move_base_client.wait_for_server(dur)
            if connected_to_move_base == True:
                self.bow_pub.publish("bow")
            # Check to make sure ROS is ok still
            if rospy.is_shutdown(): return False
            # Update the user on the status of this process
            msg = "Path Planner: waiting on move_base."
    ...
    def get_start_end_line(self,req):
        rospy.loginfo("Called, will be returned the expect value")
        current_waypoint_index = self.get_next_waypoint_index()
        rospy.loginfo("Called, current_waypoint_index is %d", current_waypoint_index)
       
        res= bowstatesResponse()
        # If the index is None, then we are done path planning
        if current_waypoint_index == None:
            rospy.loginfo("Path Planner: Done.")          
        ...
        current_waypoint = self.path[current_waypoint_index]
        current_waypoint_status = self.path_status[current_waypoint_index]
        rospy.loginfo("Called, current_waypoint_status is %s", current_waypoint_status)
        ...
        #if current_waypoint_index == 0 or current_waypoint_index == 1:
        if current_waypoint_index == 1:
            res.is_bowstate = True
            #log.info("current_waypoint_index is ")
            #log.info(current_waypoint_index)
            #log.info(current_waypoint_status)

            res.start=req.pos
            #res.start.pose=req.
            end = MoveBaseGoal()
            #end = PoseStamped()
            #rospy.loginfo("Got the request, has assigned it to response's start")
            end.target_pose.header.frame_id = self.field_frame_id
            end.target_pose.header.stamp = rospy.Time.now()
            # Set the target location
            end.target_pose.pose.position.x = current_waypoint[0]
            end.target_pose.pose.position.y = current_waypoint[1]
            #rospy.loginfo("Get the request, and assign it to response's start one")
            res.goal = end.target_pose        
            #rospy.loginfo("Get the request, and assign it to response's start second")
            #log.info("current_waypoint_index is 0 or 1")
            #log.info("The robot'pos from req is -=-=-=-=-=-=-")
            #log.info(req.pos)
            #log.info("The res.start is -=-=-=-=-=-=-=-=-=-=-=")
            #log.info(res.start)
            #log.info("The res.goal  is -=-=-=-=-=-=-=-=-=-=-=")
            #log.info(res.goal)
            
            #log.info("The start of waypoint is", res.start)
            #log.info("The goal of waypoint is ", res.goal)
            return res
        ...
        if current_waypoint_status == 'visited':
            ...
            pass
        # If the status is not_visited then we need to push the goal
        if current_waypoint_status == 'not_visited':  #or current_waypoint_status == "visiting":
            #log.info("current_waypoint_index is ")
            #log.info(current_waypoint_index)
            #log.info(current_waypoint_status)
            ...
            if(self.previous_destination==None):
                ...
                res.start = self.previous_destination.target_pose
                res.goal   = destination.target_pose
            #rospy.loginfo("Called, has been returned the computed value")
            #log.info("current_waypoint_index is larger than 1")
            #log.info("The robot'pos from req is -=-=-=-=-=-=-")
            #log.info(req.pos)
            #log.info("The res.start is -=-=-=-=-=-=-=-=-=-=-=")
            #log.info(res.start)
            ...
            #log.info(res.goal)
            return res
            # Send the start and end to for roservice server
            #rospy.loginfo("Sending waypoint (%f, %f)@%f" % tuple(current_waypoint))
