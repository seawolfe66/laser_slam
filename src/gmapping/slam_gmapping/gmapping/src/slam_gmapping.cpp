...
void SlamGMapping::mapInfoPublishInit( )
{
...
void SlamGMapping::startLiveSlam()
{
  ...
  mapinfo_publisher_ = node_.advertise<nav_msgs::MapMetaData>("map_metadata", 1, true);
...
/*!
* the LaserPose in base_frame should be static ,get it only once in this program.
*/
bool
SlamGMapping::getLaserPose(GMapping::OrientedPoint& laser_pose,const ros::Time& t)
{
    /*! Get the laser's pose, relative to base.
    * the ident is the origin of coordinates in laser_frame,
    * so transform the ident from laser_frame to base_frame meaning the laser pose relative to base.
    */
  tf::Stamped<tf::Transform> laser_pose_tf;
  tf::Stamped<tf::Pose> ident;
  ident.setIdentity();
  ident.frame_id_ = laser_frame_;
  ident.stamp_ = t;//scan.header.stamp;
  try
  {
    tf_.transformPose(base_frame_, ident, laser_pose_tf);
  }
  ...
  if(!checkForLaserMountPlane(laser_pose_tf,t))
    return false;

  double yaw = tf::getYaw(laser_pose_tf.getRotation());
  laser_pose = GMapping::OrientedPoint(laser_pose_tf.getOrigin().x(),
                                      laser_pose_tf.getOrigin().y(),
                                      yaw);
  return true;
}
bool
SlamGMapping::getOdomPose(GMapping::OrientedPoint& odom_pose, const ros::Time& t)
{
  //! Get the pose of the  base center of robot at the right time, the base pose relative odom_frame.
  tf::Stamped<tf::Transform> odom_pose_tf;
  tf::Stamped<tf::Pose> ident;
  ident.setIdentity();
  ident.frame_id_ = base_frame_;
  ident.stamp_ = t;

  try
  {
    tf_.transformPose(odom_frame_, ident, odom_pose_tf);
  }
  catch(tf::TransformException e)
  {
  ...
  double yaw = tf::getYaw(odom_pose_tf.getRotation());

  odom_pose = GMapping::OrientedPoint(odom_pose_tf.getOrigin().x(),
                                      odom_pose_tf.getOrigin().y(),
                                      yaw);
  return true;

}
void
SlamGMapping::checkForLaserRangeReverse(double scan_angle_min ,double scan_angle_max)
{

  //! we should feed the alogrithm with the angle increment in count-clockwise, this is decided by the right-hand coordinate system
  if (laser_upwards_)
  {
    do_reverse_range_ = scan_angle_min > scan_angle_max;
    ROS_INFO("Reverse the laser range.");
  }
  else
  {
    do_reverse_range_ = scan_angle_min < scan_angle_max;
...
bool
SlamGMapping::checkForLaserMountPlane(tf::Stamped<tf::Transform> & laser_pose, const ros::Time& t)
{
  /*! create a point 1m above the laser position in base_frme and transform it into the laser_frame.
  * this is for checking if the laser is mounted planar,
  * in other words, the tf between laser_frame and base_frame is only  translation(x,y,z) and horizontal rotating(yaw).
  * that is, if there is any roll and pitch , in base_frame ,the point right above laser is not right above origin in laser frame.
  */
  tf::Vector3 v;
  v.setValue(0, 0, 1 + laser_pose.getOrigin().z());
  tf::Stamped<tf::Vector3> up(v,t,base_frame_);
  try
  {
  ...
  {
    ROS_INFO("Laser has to be mounted planar,check for the realtion between laser_frame and base_frame!");
    return false;
  }
...
{
  //! init from first laserScan, setting maxRange and maxUrange here so we can set a reasonable default
  ros::NodeHandle private_nh_("~");
  ...
  //! the frame and range size should be constant !
  laser_frame_ = scan.header.frame_id;
  laser_beam_count_ = scan.ranges.size();
  laser_ranges_ = new double[laser_beam_count_];
  ROS_INFO("laserParameters[%d,%f,%f]",laser_beam_count_,scan.angle_increment,scan.angle_min);


  //! call laser_base_pose function only once
  GMapping::OrientedPoint laser_base_pose;
  if(!getLaserPose(laser_base_pose,scan.header.stamp))
    return false;
  ROS_INFO("laser_base_Pose[%f,%f,%f]",laser_base_pose.x,laser_base_pose.y,laser_base_pose.theta);

  //! check for positive angle increment,If the actual increment is negative, we'll swap the order of ranges before
  // feeding each scan to GMapping.
  checkForLaserRangeReverse(scan.angle_min,scan.angle_max);

  //! @todo: improve here
  //! The laser must be called "FLASER".
  /*! init RangeSensor
  * the RangeSensor should be init with the laser pose relative base, which is used in scanmatcher.
  */
  ...
  // the particle pose meaning the base pose in map , so init from base_odom pose.
  if(!getOdomPose(initialPose, scan.header.stamp))
  {
    ROS_WARN("Unable to determine inital pose of particles! Starting point will be set to zero.");
    initialPose = GMapping::OrientedPoint(0.0, 0.0, 0.0);
  }
  ROS_INFO("Initial particles pose:[%f,%f,%f]",initialPose.x,initialPose.y,initialPose.theta);
  ...
  gsp_->setgenerateMap(true);  //! if set true ,the mapper will rigisered all cells in map ,instead of rebuild from tree data.
...
bool
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& base_odom_pose)
{
  if(!getOdomPose(base_odom_pose, scan.header.stamp))
     return false;

  if(scan.ranges.size() != laser_beam_count_)
    return false;

  //ROS_INFO("processing odom");
  if(!gsp_->processOdom(base_odom_pose,scan.header.stamp.toSec()))
  {
    return false;
  }
  else
  {
    // In ROS ,we using right-handed coordinate system, If the angle increment is clockwise, we have to invert the order of the readings.
    if (do_reverse_range_)
    {
      ROS_DEBUG("Inverting scan");
      for(int i=0; i < laser_beam_count_; i++)
      {
          laser_ranges_[i] = (double)scan.ranges[laser_beam_count_ - i - 1];
      }
    }
    else
    {
      for(unsigned int i=0; i < laser_beam_count_; i++)
      {
          laser_ranges_[i] = (double)scan.ranges[i];
      }
    }


    ROS_INFO("processing scan");
    bool processed = gsp_->processScan(laser_ranges_,laser_beam_count_);
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  if(addScan(*scan, odom_pose))
  {
    ...
    if(!got_map_ || (scan->header.stamp - last_map_update) > map_update_interval_)
    {
      ...
      ROS_DEBUG("not time to process scan");
}
...
SlamGMapping::publishMap()
{
  ...
  //check for the & is right?
  //! get the map from best particle
  ...
  //GMapping::ScanMatcherMap &smap = (gsp_->getParticles()[gsp_->getBestParticleIndex()]).map;
  //check for this ,the map info should be updated with real value
  ...
  //check for this, map data in cell shoulb between 0-255? here is only -1,0,1
  //! fill in map data
  ...
  mapinfo_publisher_.publish(map_.map.info);

  ROS_INFO("map memory:%f Mbytes",(double)smap.getMapSizeX()*smap.getMapSizeY()*sizeof(GMapping::PointAccumulator)/1024/1024);
  ROS_INFO("Update map done!");
}
