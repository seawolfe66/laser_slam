...
# endif
  ...
  bool GridSlamProcessor::processOdom(const OrientedPoint currPose, double curr_time){
    if (!m_readingCount){
      m_lastPartPose=m_odoPose=currPose;
    }
    m_readingCount++;
    ...
    for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
      ...
      pose=m_motionModel.drawFromMotion(it->pose, currPose, m_odoPose);
    }
    ...
    OrientedPoint move=currPose-m_odoPose;
    ...
    if (m_linearDistance>m_distanceThresholdCheck){
     ...
     << " " <<m_odoPose.theta << endl;
      cerr << "New Odometry Pose (reported from observation)= " << currPose.x << " " << currPose.y
     << " " <<currPose.theta << endl;
      cerr << "***********************************************************************" << endl;
    ...
    if (! m_count
        ...
        || (period_ >= 0.0 && (curr_time - last_update_time_) > period_))
    {
      ...
      if (m_infoStream){
        ...
        //cerr << "Laser Pose= " << endl;
      }

        //! it's ok to process laserScan
        return true;
    }
    //! it's not ok to process laserScan
    return false;
  }

  bool GridSlamProcessor::processScan(const double* plainReading, int range_size, int adaptParticles){

    //this is for converting the reading in a scan-matcher feedable form
    assert(range_size == m_beams);
    ...
    if (m_count>0){
      ...
      //if (m_outputStream.is_open()){
      //  m_outputStream << "LASER_READING "<< range_size << " ";
      //  m_outputStream << setiosflags(ios::fixed) << setprecision(2);
      //  for (int i=0;i < range_size; ++i){
      //    m_outputStream << plainReading[i] << " ";
      //  }
      //  m_outputStream << setiosflags(ios::fixed) << setprecision(6);
