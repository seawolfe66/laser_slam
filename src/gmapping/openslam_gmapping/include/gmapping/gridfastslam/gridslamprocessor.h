  ...
  */
   ...
   */
    ...
    */
    ...
    bool processOdom(const OrientedPoint currPose, double curr_time);
    bool processScan(const double*  plainReading,int range_size, int adaptParticles=0);
  ...
  protected:
    ...
    //sets the neff based resampling threshold
...
inline void GridSlamProcessor::scanMatch(const double* plainReading){
  ...
  //#pragma omp parallel for reduction(+ : sumScore)//shared(plainReading) private(i) //reduction(+ : sumScore)
  for (i = 0; i < size; i++)
  {
    ...
    if (score>m_minimumScore){
      ...
      if (m_infoStream){
        m_infoStream << "Particle "<<i<<": Scan Matching Failed, using odometry. Likelihood=" << l <<std::endl;
  ...
  if (m_infoStream) {
    ...
    m_infoStream << "Scan Matching Time=" << duration << "s" << std::endl;
  }
}
...
inline bool GridSlamProcessor::resample(const double* plainReading, int adaptSize, const RangeReading* reading){
  ...
  if (m_neff<m_resampleThreshold*m_particles.size()){
    ...
    //    cerr << "Existing Nodes:" ;
    // m_index is to index the resample particle index, j is to index the  particle need to deleted
    for (unsigned int i=0; i<m_indexes.size(); i++){
      //std::cerr << " " << m_indexes[i];
      while(j<m_indexes[i]){
      ...
      TNode* oldNode=oldGeneration[m_indexes[i]];
           // std::cerr << i << "->" << m_indexes[i] << "B("<<oldNode->childs <<") ";
    ...
    m_particles.clear();
    //std::vector<Particle>().swap(m_particles);  //forces a reallocation
    std::cerr << "Done" << std::endl;
