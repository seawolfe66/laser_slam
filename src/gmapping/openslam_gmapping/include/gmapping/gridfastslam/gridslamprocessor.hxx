...
inline void GridSlamProcessor::scanMatch(const double* plainReading){
  ...
  #pragma omp parallel for shared(plainReading) private(i) reduction(+ : sumScore)
  for (i = 0; i < size; i++)
  {
    ...
    m_infoStream << "Particle "<< i << " processed! " << std::endl;
  }
  #pragma omp barrier
  ...
  if (m_infoStream)
    m_infoStream << "Average Scan Matching Score=" << sumScore/m_particles.size() << std::endl<<\
                 "time cost =" << endTime-beginTime << std::endl;
}
