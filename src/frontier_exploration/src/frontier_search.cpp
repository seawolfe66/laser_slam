...
FrontierSearch::FrontierSearch(costmap_2d::Costmap2D &costmap, std::vector<geometry_msgs::Point>& frontier_new) : costmap_(costmap),frontier_point_new(frontier_new) { VLOG(1)<<"enter into >FrontierSearch >input frontier_point_new"; }

std::list<Frontier> FrontierSearch::searchFrom(geometry_msgs::Point position){

    std::list<Frontier> frontier_list;
   // frontier_point_new.clear();
    VLOG(1)<<"enter into >searchFrom >frontier_list";
    ...
    ROS_ERROR(">>>start search");
    VLOG(1)<<"enter into >searchFrom >start search";
    //make sure map is consistent and locked for duration of search
    boost::unique_lock < costmap_2d::Costmap2D::mutex_t > lock(*(costmap_.getMutex()));
    VLOG(1)<<"enter into >searchFrom >lock costmap";
    ...
    while(!bfs.empty()){
        ...
        BOOST_FOREACH(unsigned int nbr, nhood8(idx, costmap_)){
            ...
            if(isNewFrontierCell(nbr,frontier_flag)){
                ...
                point.y = wy;
                point.z = 0;
                frontier_point_new.push_back(point);
            }
        }
    }
...
bool FrontierSearch::isNewFrontierCell(unsigned int idx, const std::vector<bool>& frontier_flag){
    ...
    int frontier_size ;
    frontier_size = frontier_flag.size();
    if( map_[idx] != 200 || frontier_flag[idx]){
        return false;
    }
