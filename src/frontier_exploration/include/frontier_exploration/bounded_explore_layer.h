...
//    bool getNextFrontierService(frontier_exploration::GetNextFrontier::Request &req, frontier_exploration::GetNextFrontier::Response &res);
    ...
    bool getCostmapPolygonService(frontier_exploration::CostmapPolygon::Request &req, frontier_exploration::CostmapPolygon::Response &res);
   ...
   // bool getNextFrontier(geometry_msgs::PoseStamped start_pose, geometry_msgs::PoseStamped &next_frontier);
    ...
    bool getCostmapPolygon(geometry_msgs::PolygonStamped costmap_polygon_stamped, geometry_msgs::PolygonStamped &sweeps_polygon_stamped);


    void syncSweepAreas(const sweeps_areas::sweeps & areas);
    bool getFrontierPolygon(geometry_msgs::Polygon &frontier_polygon);
...
private:
    ...
    ros::ServiceServer costmapPolygonService_;
    geometry_msgs::Polygon polygon_;
    geometry_msgs::Polygon frontier_polygon_;
    visualization_msgs::Marker points_list, line_strip_list;
    tf::TransformListener tf_listener_;

    ros::Publisher frontier_cloud_pub;
    //ros::Publisher frontier_marker_pub_;
    sweeps_areas::sweeps sweeps_areas_polygon;
    ros::Subscriber sweeps_areas_sub_; 
    std::vector<geometry_msgs::Point> frontier_list_new;
    std::vector<geometry_msgs::Point> frontier_list_copy; 

    //ros::Publisher frontier_point_cloud_pub;
    ros::Publisher frontier_point_cloud_pub1;
    ros::Publisher costmap_sweeps_point_cloud_pub;
    ros::Publisher point_viz_pub_;
    ros::Publisher rectanglePolygon_pub_;
    geometry_msgs::PoseStamped start_pose;
    geometry_msgs::PolygonStamped input_rectangle_polygon_;
   ...
   // unsigned int min_sweeps_x;
   // unsigned int max_sweeps_x;
    //unsigned int min_sweeps_y;
    //unsigned int max_sweeps_y;
    int sweeps_points_num;
    //sweeps_points_num = 0;
    std::list<Frontier> frontier_list;
    //std::list<Frontier> frontier_list1;
   // std::list<Frontier> frontier_list2;
    geometry_msgs::Polygon output_polygon_; 
};
