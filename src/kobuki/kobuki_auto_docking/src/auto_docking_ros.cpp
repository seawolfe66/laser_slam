  ...
  , as_(nh_, name_+"_action", false)
{
  ...
  ROS_ERROR("hello, autodocking is created");
  self = this;
  actionStop = false; 
  ...
  , as_(nh_, name_+"_action", false)
{
  ...
  ROS_ERROR("hello, autodocking is created , originally");
  self = this;
  actionStop = false; 
  as_.registerGoalCallback(boost::bind(&AutoDockingROS::goalCb, this));
  as_.registerPreemptCallback(boost::bind(&AutoDockingROS::preemptCb, this));
  ...
  ROS_ERROR("end of created originally");
}

AutoDockingROS::~AutoDockingROS()
{
  ROS_ERROR(">>>>>AutoDockingROS::~AutoDockingROS");
...
/** add for status machine by chenrui*/
std::string AutoDockingROS::getStateStr(){
    return dock_.getStateStr();
}


RobotDockingState::State  AutoDockingROS::getState(){

    return dock_.getState();
...
bool AutoDockingROS::init(ros::NodeHandle& nh)
{
  ...
  // ros::Subscriber docking_command;
  status_pub_ = nh.advertise<std_msgs::String>("/dock_status", 1);
...
void AutoDockingROS::setPrivateHandler(ros::NodeHandle  nh){
    private_nh = nh; 
}
...
{
  ROS_ERROR(">>>>>chenrui>>>>AutoDockingROS::goalCb");
  if (dock_.isEnabled()) {
                            ...
                            const kobuki_msgs::DockInfraRedConstPtr& ir)
{
  if(actionStop)
       return; 
  ROS_ERROR(">>>>chenrui>>>AutoDockingROS::syncCb");
  //process and run
...
//  if( as_.isActive() ) {
    if ( dock_.getState() == RobotDockingState::DONE ) {
     ...
     // ROS_DEBUG_STREAM( "[" << name_ << "]: Result sent.");
      //added by chenrui
  /*    std_msgs::StringPtr status_ok(new std_msgs::String);
      status_ok->data = "DONE";
      status_pub_.publish(status_ok);
