...
MultiMapper::MultiMapper()
{
	...
	mapperNode.param("min_radius",minRadius,0.2);
...
karto::LocalizedRangeScan* MultiMapper::createFromRosMessage(const sensor_msgs::LaserScan& scan, const karto::Identifier& robot)
{
	...
	double filter[scan.ranges.size()];
        #if 0 
	//laser filter
	for(int i=0;i<scan.ranges.size();i++){	
		if(filter[i]>=scan.range_min&&filter[i]<=scan.range_max)
			filter[i]=scan.ranges[i];
		else if(filter[i]<scan.range_min||filter[i]>scan.range_max){
			if(i<2)
				filter[i]=(scan.ranges[i+1]+scan.ranges[i+2])/2;
			else 
				filter[i]=(scan.ranges[i-2]+scan.ranges[i-1])/2;
		}
		else
			filter[i]=scan.range_max;
			readings.Add(filter[i]);	
	}	
	#else
	for(it = scan.ranges.begin(); it != scan.ranges.end(); it++)
	{
		
	 	//ROS_INFO("scan info is %f",*(it+1));
...
void MultiMapper::receiveLaserScan(const sensor_msgs::LaserScan::ConstPtr& scan){
	...
	if(mState == ST_MAPPING)
	{
		...
		//ROS_INFO("Robot position x  is: %f,y is :%f",tfPose.getOrigin().x(),tfPose.getOrigin().y());
		karto::Pose2 kartoPose = karto::Pose2(tfPose.getOrigin().x(), tfPose.getOrigin().y(), tf::getYaw(tfPose.getRotation()));
		if(!first){
			first=true;
			robot_pose.x=tfPose.getOrigin().x();
			robot_pose.y=tfPose.getOrigin().y();
		}
		if(cout>4){
			if(fabs(tfPose.getOrigin().x()-robot_pose.x)<minRadius&&fabs(tfPose.getOrigin().y()-robot_pose.y)<minRadius){
				enable_mapping=false;
				//first=false;
				//ROS_INFO("you just draw a circle");
			}
		}
