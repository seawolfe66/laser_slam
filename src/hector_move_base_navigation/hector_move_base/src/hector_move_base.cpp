...
HectorMoveBase::HectorMoveBase(std::string name, tf::TransformListener& tf) :
    costmap_(NULL),control_costmap_(NULL),
    nh_(""),
    private_nh_("~"),
    tf_(tf),
    ...
    action_server_(nh_, name, false)
{
    ...
    control_costmap_= new costmap_2d::Costmap2DROS("local_costmap", tf_);
    costmap_->pause();
    control_costmap_->pause();
    ...
    //chenrui,,here, we should accept the battery information , and if it is low battery , we should start the autodocking
   // battery_subscriber = nh_.subscribe<kobuki_msgs::PowerSystemEvent>("/mobile_base/events/power_system", 10, boost::bind(&HectorMoveBase::BatteryCallback, this, _1)); 
...
/*
void HectorMoveBase::BatteryCallback(const  kobuki_msgs::PowerSystemEventConstPtr &msg){

       ROS_ERROR(">>>>>chenrui>>>>battery call back>>>>");

       if(msg->event == kobuki_msgs::PowerSystemEvent::BATTERY_LOW){ //if battery is low, then start to search the docker; here, there is complex logic.

       ROS_ERROR(">>>>>chenrui>>>>battery low>>>>");
           setNextState(autodockingState_);

       }else if(msg->event == kobuki_msgs::PowerSystemEvent::PLUGGED_TO_DOCKBASE){//here, we need to stop the auto docking process
...
HectorMoveBase::~HectorMoveBase()
{
    if(costmap_ != NULL)
        delete costmap_;
    if(control_costmap_!=NULL)
        delete control_costmap_;
}
...
void HectorMoveBase::setupStateMachine()
{
    ...
    dwaMoveState_.reset(new hector_move_base_handler::HectorMoveDwaHandler(this));
    
    ROS_ERROR(">>>>will create hector_move_base_handler::HectorAutoDockingHandler");

    //autodockingState_.reset(new hector_move_base_handler::HectorAutoDockingHandler(this)); //add by chenrui for autodocking 
    ROS_ERROR("[hector_move_base] All states created");
...
//    statemachine_->addHandlerMapping(publishPathState_, mappingForPublishPath);
    ...
    std::map<RESULT, boost::shared_ptr<hector_move_base_handler::HectorMoveBaseHandler> > mappingForPublishPath;
    mappingForPublishPath.insert(std::make_pair(NEXT, dwaMoveState_));
    statemachine_->addHandlerMapping(publishPathState_, mappingForPublishPath);

    std::map<RESULT, boost::shared_ptr<hector_move_base_handler::HectorMoveBaseHandler> > mappingForDwaMove;
    mappingForDwaMove.insert(std::make_pair(NEXT, publishFeedbackState_));
    mappingForDwaMove.insert(std::make_pair(FAIL, planningState_));
    mappingForDwaMove.insert(std::make_pair(SUCCESS, idleState_));
    statemachine_->addHandlerMapping(dwaMoveState_, mappingForDwaMove);
    ...
    mappingForWaitForReplanning.insert(std::make_pair(NEXT, dwaMoveState_));
    ...
    //add begin for autodocking 
/*    std::map<RESULT, boost::shared_ptr<hector_move_base_handler::HectorMoveBaseHandler> > mappingForAutodockingState;
    mappingForAutodockingState.insert(std::make_pair(NEXT, autodockingState_)); //chenrui , here, perhaps need to be modified.
    statemachine_->addHandlerMapping(autodockingState_, mappingForAutodockingState);

*/
    //add end for autodocking

    ROS_DEBUG("[hector_move_base] State machine mapping created");

    startState_ = planningState_;
...
void HectorMoveBase::setCurrentState(boost::shared_ptr<hector_move_base_handler::HectorMoveBaseHandler> nextState) {
    ROS_DEBUG("[hector_move_base]: setCurrentState()");
    currentState_= nextState;
   // ROS_INFO("The type of nextstate is %s ",nextState.get());
}
...
costmap_2d::Costmap2DROS* HectorMoveBase::getControlCostmap() {
    return control_costmap_;
}
...
void HectorMoveBase::simple_goalCB(const geometry_msgs::PoseStampedConstPtr &simpleGoal) {
    ...
    pushCurrentGoal(newGoal);
    setNextState(planningState_);
    //setCurrentState(planningState_);
    return;
}
...
void HectorMoveBase::controllerResultCB(const hector_move_base_msgs::MoveBaseActionResultConstPtr &result) {
    ...
    case actionlib_msgs::GoalStatus::PREEMPTED:
        ...
        if ( (currentState_ == stuckPlanningRecoveryState_) ||  (nextState_ == stuckPlanningRecoveryState_)) {
            return;
        }
        preemptedGoal();
        return;
...
/*    if(currentState_ == autodockingState_ && result == hector_move_base::FAIL){
